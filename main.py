...
def draw_window(red, yellow, red_bullet, yellow_bullet, red_health, yellow_health):
    ...
    red_health_text = HEALTH_FONT.render("Amirtoy: " + str(red_health), 1, WHITE)
    yellow_health_text = HEALTH_FONT.render("Alimoff: " + str(yellow_health), 1, WHITE)
...
def yellow_handle_movement(key_pressed, yellow):
    if key_pressed[pygame.K_a] and yellow.x - VEL > 0:  # LEFT
        yellow.x -= VEL
    if (
        key_pressed[pygame.K_d] and yellow.x + VEL + yellow.width < BORDER.x + 15
    ):  # RIGHT
        yellow.x += VEL
...
def red_handle_movement(key_pressed, red):
    if key_pressed[pygame.K_LEFT] and red.x - VEL > BORDER.x + BORDER.width:  # LEFT
        red.x -= VEL
    if key_pressed[pygame.K_RIGHT] and red.x + VEL + red.width < WIDTH + 20:  # RIGHT
        red.x += VEL
...
def main():
    ...
    while run:
        ...
        draw_window(red, yellow, red_bullets, yellow_bullet, red_health, yellow_health)
